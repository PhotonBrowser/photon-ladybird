...
Use `./photon run --dev` (or the `./photon run dev` shorthand) for live frontend work. Dev mode installs dependencies when needed, starts Vite on `127.0.0.1:5173`, and runs Photon against that server instead of the production bundle, so it never runs `npm run build`.
