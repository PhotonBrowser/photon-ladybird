...
The links point to Git worktrees in the sibling `.<checkout>-photon-worktrees/` directory, based on the recorded upstream revision. Keeping their actual paths outside the canonical checkout prevents Cargo from discovering the wrong Rust workspace. Their `Build` directories point at separate persistent locations in a sibling `.<checkout>-photon-build/` directory (`PhotonBuild` and `PhotonEdit`) so CMake caches stay tied to their source roots. Removing generated source does not delete build artifacts. If `.photon/worktree` exists, build, run, and test use it so engine edits can be built before capture; otherwise they use `Build/Source`.
